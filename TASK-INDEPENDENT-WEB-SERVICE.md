...
7. 服务器不永久保存视频；未领取文件 30 分钟删除，首次下载请求后 15 分钟删除。
...
- [x] 严格 JSON 请求校验。
- [x] React 前端接入同源 API。
- [x] 深色/浅色主题；嵌入 XimoAI 主站时通过 `preferences:update` 实时同步，独立打开时使用本地持久化主题。
- [x] 移除独立顶部导航，将临时文件生命周期提示迁移到主内容区显著位置。
...
2. 在低流量时段执行生产限流、取消和 15/30 分钟 TTL 演练。
