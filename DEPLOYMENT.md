...
| 文件已过期 | 首次下载后超过 10 分钟或未领取超过 30 分钟 |
| 磁盘增长 | 活动任务、容器重启、`cache` 权限和清理循环 |
| 主题仍是旧版 | 浏览器/CDN 缓存和实际运行镜像 ID |

## 19. 正式上线验收

- [ ] A 记录只指向 `47.251.87.147`。
- [ ] 正式地址为 `https://video-collector.ximoai.cn`。
- [ ] HTTPS 有效并自动续期。
- [ ] 8787 只绑定本机。
- [ ] Compose 配置展开通过。
- [ ] 容器状态为 `healthy`。
